...
class OutBreakStates():
    TABLE = "test" # "prod"
    DATETIME_FORMAT = "%Y-%m-%d %H:%M:%S"
    HEADER = {"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_3) AppleWebKit/537.36 (KHTML, like Gecko)"}
    def __init__(self):
        self.db_malaysia_patient_cases = MySQL_Connector()
        self.db_malaysia_states= MySQL_Connector()
    ...
    def get_state_details(self):
        ...
        db_malaysia_states.insert(target_table=db_queries['malaysia_states'][TABLE], query=db_queries['malaysia_states']['query'], data_dict=data_dict)
        ...
        for data in raw:
            ...
            while idx < len(data):
                ...
                if idx == 0 and "case" in val.lower():
                    ...
                    patient["status_date"] = parser.parse(date).strftime(DATETIME_FORMAT)
            ...
            db_malaysia_patient_cases.insert(target_table=db_queries['malaysia_patient_case'][TABLE], query=db_queries['malaysia_patient_case']['query'], data_dict=patient)
        # print(patients)


if __name__ == "__main__":
    outbreak = OutBreakStates()
    OutBreakStates.get_state_details()
    OutBreakStates.get_case_details()
